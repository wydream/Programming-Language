...
fun eval_prog (e,env) =
	...
	(case List.find (fn (s2,v) => s=s2) env of
      ...
      let val ee = eval_prog (e, env) in
      	case ee of 
      		NoPoints => ee
      		| Point(x,y) => Point(x+deltaX, y+deltaY)
      		| Line(slope, intercept) => Line(slope, intercept + deltaY - intercept * deltaX)
...
fun preprocess_prog (e) =
	case e of
		...
		else if real_close(x1,x2) andalso (y1>y2) orelse (x1>x2) then LineSegment(x2,y2,x1,y1)
		else e
		| _ => e
