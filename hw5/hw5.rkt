  ...
  (cond [(var? e) 
        ...
        [(call? e)
         (let ([cl (eval-under-env (call-funexp e) env)]
               [ca (eval-under-env (call-actual e) env)])
           (if (closure? cl)
           (let ([f (closure-fun cl)]
                 [cenv (closure-env cl)])
             (eval-under-env (fun-body f)
                             (if (fun-nameopt f)
                                 (cons (cons (fun-formal f) ca) (cons (cons (fun-nameopt f) cl) cenv))
                                 (cons (cons (fun-formal f) ca) cenv))))
           (error "MUPL call applied to non-closure")))]
        [(apair? e)
         (apair (eval-under-env (apair-e1 e) env) (eval-under-env (apair-e2 e) env))]
        [(fst? e)
         (let ([ap (eval-under-env (fst-e e) env)])
           (if (apair? ap)
               (apair-e1 ap)
               (begin (print ap)
               (error "MUPL fst applied to non-apair"))))]
        [(snd? e)
         (let ([ap (eval-under-env (snd-e e) env)])
           (if (apair? ap)
               (apair-e2 ap)
  ...
  (fun #f "mf"
       (fun "loop" "mxs"
            (ifaunit (var "mxs")
                     (aunit)
                     (apair
                      (call (var "mf") (fst (var "mxs")))
                      (call (var "loop") (snd (var "mxs"))))))))
